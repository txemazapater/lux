...
Phases 0–5, 5.1, 5.2, 6A and **6B.1** (stack) are complete. Continue Phase 6B via ROADMAP subphases **6B.1–6B.5** (next: **6B.2 logical cursor**). Do not start 6B.3+ until 6B.2 has tests, a dedicated demo, and maintainer visual sign-off. Phase 5.2.1 deferred resize remains experimental. Keep ANSI generation and the differential renderer portable. Keep Win32 in `src/platform/windows` and termios in `src/platform/unix`. Keep controls in `src/controls` and layouts in `src/layouts` free of platform units. Full repaint means rewrite all cells, not erase the terminal (`docs/adr/0006-full-repaint-without-clear.md`). Layout model: `docs/adr/0008-layout-model.md`. Cursor: `docs/adr/0009-logical-cursor-manager.md`.
