...
Phases 0–5, 5.1, 5.2, 6A, **6B.1–6B.4**, **6C**, and **6D** are implemented. **Implementation is paused for architecture review** — see `docs/architecture-review-2026-07.md`. Do **not** start old Phase 6E (ProgressBar/Slider) without a Range model; preferred next slice is **S1** (client area + thin appearance seam). Full Appearance (`.luxtheme`) remains later (`docs/adr/0013-appearance-themes-glyphs-config.md`). Former “Developer usability” is Phase 8; Phase 9 is consumer readiness (e.g. for a future Lantern app — not to be confused with **Lighthouse**, the commercial name of LUX). Phase 5.2.1 deferred resize remains experimental. Keep ANSI generation and the differential renderer portable. Keep Win32 in `src/platform/windows` and termios in `src/platform/unix`. Keep controls in `src/controls` and layouts in `src/layouts` free of platform units. Full repaint means rewrite all cells, not erase the terminal (`docs/adr/0006-full-repaint-without-clear.md`). Layout: `docs/adr/0008-layout-model.md`. Cursor: `docs/adr/0009-logical-cursor-manager.md`. Split/capture: `docs/adr/0010-split-container-mouse-capture.md`. Mouse dispatcher + ScrollView: `docs/adr/0011-mouse-dispatcher-scrollview.md`. Form controls: `docs/adr/0012-form-controls-checkbox-radio.md`, `docs/phase-6d-form-controls.md`, GroupBox: `docs/adr/0014-groupbox-client-area.md`. Appearance (planned): `docs/adr/0013-appearance-themes-glyphs-config.md`.
