...
Phases 0–5, 5.1, 5.2, 6A, **6B.1–6B.4**, **6C**, and **6D** are complete with visual review passed. Continue remaining Phase 6 (deferred form controls / **6E** onward). **Phase 7 — Appearance System** is planned after Phase 6 completes (`docs/adr/0013-appearance-themes-glyphs-config.md`); do not start it while Phase 6 is open. Former “Developer usability” is Phase 8; “Lantern readiness” is Phase 9. Phase 5.2.1 deferred resize remains experimental. Keep ANSI generation and the differential renderer portable. Keep Win32 in `src/platform/windows` and termios in `src/platform/unix`. Keep controls in `src/controls` and layouts in `src/layouts` free of platform units. Full repaint means rewrite all cells, not erase the terminal (`docs/adr/0006-full-repaint-without-clear.md`). Layout: `docs/adr/0008-layout-model.md`. Cursor: `docs/adr/0009-logical-cursor-manager.md`. Split/capture: `docs/adr/0010-split-container-mouse-capture.md`. Mouse dispatcher + ScrollView: `docs/adr/0011-mouse-dispatcher-scrollview.md`. Form controls: `docs/adr/0012-form-controls-checkbox-radio.md`. Appearance (planned): `docs/adr/0013-appearance-themes-glyphs-config.md`.
